...
ARG IMAGE_BUILDER_DIGEST='sha256:adbb90115a21969d2fe6fa7f9af4253e16d45f8d4c1e930182610c4731962658'
...
RUN --mount=type=cache,target=/home/${PYTHON_BUILDER_USER}/.cache/compiled_python,uid=${PYTHON_BUILDER_USER_ID},gid=${PYTHON_BUILDER_USER_ID} \
    --mount=type=cache,target=/home/${PYTHON_BUILDER_USER}/.cache/models,uid=${PYTHON_BUILDER_USER_ID},gid=${PYTHON_BUILDER_USER_ID} \
    chmod a=rxw .; \
    ${RUN_PYTHON} -m poetry run bash -c "python3 /usr/src/app/src/_download_metric_models.py '/home/${PYTHON_BUILDER_USER}/.cache/models'"; \
    rm /usr/src/app/src/_download_metric_models.py; \
    ls -la '/usr/src/app/metric_models_dir'; \
    ls -la '/usr/src/app/metric_models_dir/assets'; \
    ls -la '/usr/src/app/metric_models_dir/hub';
...
COPY --link --chown=root:root ./static/ "${WORK_DIR}/static"
