...
from os import getgid, getlogin, getuid  # noqa: E402
from pathlib import Path  # noqa: E402
from subprocess import PIPE, Popen  # noqa: E402, S404
from sys import argv, stdout  # noqa: E402
...
from l7x.utils.orjson_utils import orjson_dumps_to_str_pretty, orjson_loads  # noqa: E402
...
def _create_envs(logger: Logger | None) -> dict[str, str]:
    ...
    return {
        ...
        'USER_NAME': user_name,
        'POSTGRES_PASSWORD': app_settings.db_admin_pass,
        'POSTGRES_USER': app_settings.db_admin_user,
        'POSTGRES_DB': app_settings.db_name,
        'POSTGRES_PORT': str(app_settings.db_port),
        'DOCKER_BUILDKIT': '1',
    }
...
def get_environment_docker_containers(logger: Logger | None = None) -> Sequence[DockerContainerInfo]:
    with Popen(  # noqa: S603
        ...
        for out_part in out_str.split(b'\n'):
            docker_container = orjson_loads(out_part)
...
def _run(logger: Logger, argv_list: Sequence[str]) -> None:
    ...
    match option:
        ...
        case 'info':
            logger.info(orjson_dumps_to_str_pretty(get_environment_docker_containers(logger)))
        case _:
