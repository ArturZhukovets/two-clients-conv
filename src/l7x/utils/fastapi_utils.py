...
async def _get_robots_txt() -> Response:
...
class AppFastAPI(FastAPI):
    ...
    def __init__(
        ...
        self._password_hasher = create_password_hasher(app_settings, logger)
        ...
        self.get('/robots.txt', include_in_schema=False)(_get_robots_txt)
...
class _AppDevAPIRoute(APIRoute):
    ...
    def get_route_handler(self) -> Callable[[_StarletteRequest], Coroutine[Any, Any, Response]]:
        ...
        async def custom_route_handler(req: _StarletteRequest) -> Response:
            ...
            try:
                res_body_str = body.decode('utf-8') if isinstance(body, bytes) else str(type(res))
            except UnicodeDecodeError:
